...
local function checkTable(key, val, xPlayer, xPlayers, minimal)
    for valIndex = 1, #val do
        ...
        local matches = (key == "job" and xPlayer.job.name == value)
            or (key == "gang" and xPlayer.gang and xPlayer.gang.name == value)
            or (key ~= "job" and key ~= "gang" and xPlayer[key] == value)
...
function ESX.GetExtendedPlayers(key, val, minimal)
    ...
    for _, xPlayer in pairs(ESX.Players) do
        local matches = (key == "job" and xPlayer.job.name == val)
            or (key == "gang" and xPlayer.gang and xPlayer.gang.name == val)
            or (key ~= "job" and key ~= "gang" and xPlayer[key] == val)
...
function ESX.GetNumPlayers(key, val)
    ...
    if type(val) == "table" then
        ...
        if key == "job" or key == "gang" then
            local counts = key == "job" and Core.JobsPlayerCount or Core.GangsPlayerCount
            for _, v in ipairs(val) do
                numPlayers[v] = (counts[v] or 0)
            end
            return numPlayers
        end
    ...
    elseif key == "gang" then
        return (Core.GangsPlayerCount[val] or 0)
    end
...
---@return nil
function Core.EnsureGangSchema()
    local gangColumn = MySQL.scalar.await([[
        SELECT COUNT(*)
        FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = 'users'
          AND COLUMN_NAME = 'gang'
    ]])

    if gangColumn == 0 then
        MySQL.update.await("ALTER TABLE `users` ADD COLUMN `gang` VARCHAR(50) NULL DEFAULT 'nogang' AFTER `job_grade`")
    end

    local gangGradeColumn = MySQL.scalar.await([[
        SELECT COUNT(*)
        FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = 'users'
          AND COLUMN_NAME = 'gang_grade'
    ]])

    if gangGradeColumn == 0 then
        MySQL.update.await("ALTER TABLE `users` ADD COLUMN `gang_grade` INT NULL DEFAULT 0 AFTER `gang`")
    end
    ...
    MySQL.query.await("INSERT IGNORE INTO `gangs` (`name`, `label`) VALUES ('nogang', 'No Gang')")
    MySQL.query.await("INSERT IGNORE INTO `gang_grades` (`gang_name`, `grade`, `name`, `label`) VALUES ('nogang', 0, 'nogang', 'No Gang')")
    MySQL.update.await("UPDATE `users` SET `gang` = 'nogang' WHERE `gang` IS NULL OR `gang` = ''")
    MySQL.update.await("UPDATE `users` SET `gang_grade` = 0 WHERE `gang_grade` IS NULL")
end
...
function ESX.RefreshGangs()
    ...
    local Gangs = {}
    local gangs = MySQL.query.await("SELECT * FROM `gangs`") or {}

    for _, gang in ipairs(gangs) do
        Gangs[gang.name] = gang
        Gangs[gang.name].grades = {}
    end

    local gangGrades = MySQL.query.await("SELECT * FROM `gang_grades`") or {}
    for _, grade in ipairs(gangGrades) do
        if Gangs[grade.gang_name] then
            Gangs[grade.gang_name].grades[tostring(grade.grade)] = grade
        else
            print(('^3[WARNING]^7 Ignoring gang grade for ^5"%s"^7 due to missing gang definition.'):format(grade.gang_name))
        end
    end

    for name, gang in pairs(Gangs) do
        if ESX.Table.SizeOf(gang.grades) == 0 then
            Gangs[name] = nil
            print(('^3[WARNING]^7 Ignoring gang ^5"%s"^7 because it has no grades.'):format(name))
        end
    end

    if not Gangs.nogang then
        Gangs.nogang = {
            name = "nogang",
            label = "No Gang",
            grades = {
                ["0"] = { gang_name = "nogang", grade = 0, name = "nogang", label = "No Gang" },
            },
        }
    end

    ESX.Gangs = Gangs
    Core.GangsLoaded = true
    TriggerEvent("esx:gangsRefreshed")
end
