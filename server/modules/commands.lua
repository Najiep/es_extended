...
ESX.RegisterCommand(
    ...
    function(xPlayer, args, showError)
        if not ESX.DoesGangExist(args.gang, args.grade) then
            return showError("The gang, grade or both are invalid")
        end
...
ESX.RegisterCommand("refreshjobs", "admin", function()
    ESX.RefreshJobs()
end, true, { help = "Refresh job definitions" })

ESX.RegisterCommand("refreshgangs", "admin", function()
    ESX.RefreshGangs()
end, true, { help = "Refresh gang definitions" })
...
ESX.RegisterCommand("gang", { "user", "admin" }, function(xPlayer)
    local gang = xPlayer.getGang()
    print(("%s, your gang is: ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), gang.name, gang.grade_label))
end, false)

ESX.RegisterCommand("info", { "user", "admin" }, function(xPlayer)
    local job = xPlayer.getJob().name
    local gang = xPlayer.getGang().name
    print(("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job: ^5%s^0 | ^2Gang: ^5%s^0"):format(xPlayer.source, xPlayer.getName(), xPlayer.getGroup(), job, gang))
end, false)
