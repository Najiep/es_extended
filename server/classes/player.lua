...
---@class ESXGang
---@field name string               # Gang internal name.
---@field label string              # Gang display label.
---@field grade number              # Current gang grade/rank number.
---@field grade_name string         # Name of the current gang grade.
---@field grade_label string        # Label of the current gang grade.
...
---@field setJob fun(newJob: string, grade: string|number, onDuty?: boolean) # Set player's job and grade.
---@field getGang fun(): ESXGang                                       # Get player's current gang.
---@field setGang fun(newGang: string, grade: string|number)            # Set player's gang and grade.
...
---@field gang ESXGang              # Player's current gang.
...
---@param gang? ESXGang
---@return xPlayer
function CreateExtendedPlayer(playerId, identifier, ssn, group, accounts, inventory, weight, job, loadout, name, coords, metadata, gang)
    ...
    self.job = job
    self.gang = gang or { name = "nogang", label = "No Gang", grade = 0, grade_name = "nogang", grade_label = "No Gang" }
    ...
    function self.setGang(newGang, grade)
        ...
        if not ESX.DoesGangExist(newGang, grade) then
            return print(("[ESX] [^3WARNING^7] Ignoring invalid ^5.setGang()^7 usage for ID: ^5%s^7, Gang: ^5%s^7"):format(self.source, newGang))
        end
