...
local loadPlayer = "SELECT `accounts`, `ssn`, `job`, `job_grade`, `gang`, `gang_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `metadata`"
...
local function onPlayerDropped(playerId, reason, cb)
    ...
    GlobalState[("gang:%s:count"):format(gang)] = Core.GangsPlayerCount[gang]
...
if Config.Multichar then
    AddEventHandler("esx:onPlayerJoined", function(src, char, data)
        while not Core.JobsLoaded or not Core.GangsLoaded do
            Wait(50)
        end
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    -- Gang
    local gang, gangGrade = result.gang or "nogang", tostring(result.gang_grade or 0)
    ...
    -- xPlayer Creation
    local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.ssn, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.loadout, GetPlayerName(playerId), userData.coords, userData.metadata, userData.gang)
...
AddEventHandler("esx:playerLoaded", function(_, xPlayer, isNew)
    ...
    local gangKey = ("gang:%s:count"):format(gang)
    ...
    GlobalState[gangKey] = Core.GangsPlayerCount[gang]
    if isNew then
...
AddEventHandler("esx:setGang", function(_, gang, lastGang)
    local lastGangKey = ("gang:%s:count"):format(lastGang.name)
    local gangKey = ("gang:%s:count"):format(gang.name)
    local currentLastGang = Core.GangsPlayerCount[lastGang.name]

    Core.GangsPlayerCount[lastGang.name] = ((currentLastGang and currentLastGang > 0) and currentLastGang or 1) - 1
